...
public class RegiInfoSource {
	...
	private String[] basicInfoNames= {InfoSQLiteHelper.COLUMN_FIRSTNAME, InfoSQLiteHelper.COLUMN_LASTNAME};
	...
	*/
	...
	public boolean createBasicInfo(String firstname, String lastname, String dob, String ssn, 
		...
		if(insertId == -1)
			...
			return true;
		/*
		Cursor cursor = db.query(InfoSQLiteHelper.TABLE_REGI, basicInfoColumns, InfoSQLiteHelper.COLUMN_ID + " = " + insertId, null, null, null, null);
	...
	public RegiInfo searchsamePatientRegiInfo(String ssn){
		//Cursor cursor = db.rawQuery("SELECT * FROM " + InfoSQLiteHelper.TABLE_REGI + " WHERE " + InfoSQLiteHelper.COLUMN_USERNAME + " ='" + username +"'", null);
		
		Cursor cursor = db.query(InfoSQLiteHelper.TABLE_REGI, basicInfoColumns, InfoSQLiteHelper.COLUMN_SSN + " = '" + ssn + "'", null, null, null, null);
		cursor.moveToFirst();
		...
		if(cursor.getCount() == 0){
			...
			RegiInfo patientinfo = cursorToBasicInfo(cursor);
			cursor.close();
			return patientinfo;
		}
	...
	//listView to get a personal basic information
	...
	public List<RegiInfo> getNamelist(){
		List<RegiInfo> regi_info = new ArrayList<RegiInfo>();
		
		Cursor cursor = db.query(InfoSQLiteHelper.TABLE_REGI, basicInfoColumns, null, null, null, null, null);
		cursor.moveToFirst();
	...
	public boolean updateRegiInfo(int id, String allergies, String medicalhistory){
	ContentValues values = new ContentValues();
	values.put(InfoSQLiteHelper.COLUMN_ALLERGIES, allergies);
	values.put(InfoSQLiteHelper.COLUMN_MEDICALHIS, medicalhistory);
	
	int row = db.update(InfoSQLiteHelper.TABLE_REGI, values, InfoSQLiteHelper.COLUMN_ID + "=" + id, null);
	...
	public boolean updateRegiInfoUsingSSN(String ssn, String allergies, String medicalhistory){
		ContentValues values = new ContentValues();
		values.put(InfoSQLiteHelper.COLUMN_ALLERGIES, allergies);
		values.put(InfoSQLiteHelper.COLUMN_MEDICALHIS, medicalhistory);
		
		int row = db.update(InfoSQLiteHelper.TABLE_REGI, values, InfoSQLiteHelper.COLUMN_SSN + "='" + ssn + "'", null);

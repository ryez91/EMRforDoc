...
import com.example.emrfordoc.R;
...
public class BluetoothActivity extends Activity {
    ...
    private String key;
    String[] messageindata;
    String[] strings;
    RegiInfoSource recdata;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		recdata = new RegiInfoSource(BluetoothActivity.this);
   	 	recdata.open();
		
   	 messageindata = new String[10];
	...
	//				Toast.makeText(MainActivity.this, "blueeeee", Toast.LENGTH_LONG).show();
	 ...
	 private final Handler mHandler = new Handler() {
	   ...
	   //             String readMessage = new String(readBuf, 0, msg.arg1);
	                ...
	                strings=readMessage.split(",,,");
	                for(int i =0;i<10;i++){
	                mConversationList.add(strings[i]);
               	 	mConversationArrayAdapter.notifyDataSetChanged();
               	 	
             	 	messageindata[i] = encryption.aesEncrypt(strings[i], key);
               	     //messageindata[i] = strings[i];
	                }
	                
	                //Beginning of update on Dec 9
	                //messagedata[3]= encrypted ssn
	                
	                RegiInfo dataexist = recdata.searchsamePatientRegiInfo(messageindata[3]);
	                if(dataexist==null){
	               	 	recdata.createBasicInfo(messageindata[0], messageindata[1], messageindata[2], messageindata[3], messageindata[4], messageindata[5], messageindata[6], messageindata[7], messageindata[8], messageindata[9]);
	               	 	
	                }else{
	                	int patientid =dataexist.getId();
	                	recdata.updateBasicInfo(patientid, messageindata[0], messageindata[1], messageindata[2], messageindata[3], messageindata[4], messageindata[5], messageindata[6], messageindata[7], messageindata[8], messageindata[9]);
	                }
	                //end of update on Dec 9.
